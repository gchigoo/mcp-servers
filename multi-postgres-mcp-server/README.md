...
A Model Context Protocol server that provides read-only access to PostgreSQL databases. This server enables LLMs to inspect database schemas and execute read-only queries, and it supports connecting to **multiple databases at once** using `id=postgres://...` arguments.
...
- **query**
  ...
  - Input:
    ...
    - `databaseId` (string, optional): Target database id; **required when multiple databases are configured**
...
- **Table Schemas**
  - Single database: `postgres://<host>/<table>/schema`
  - Multiple databases: `postgres://<databaseId>/<table>/schema`
  - JSON schema information for each table
  - Includes column names and data types
  - Automatically discovered from database metadata

When multiple databases are provided, resource names include the database id (e.g., `[db1] "users" database schema`).
...
To connect to multiple databases with Docker, pass `id=url` pairs:
  ...
  "mcpServers": {
    "postgres": {
      ...
      "args": [
        ...
        "--rm",
        "mcp/postgres",
        "db1=postgresql://host.docker.internal:5432/db_one",
        "db2=postgresql://host.docker.internal:5432/db_two"
      ]
    }
  }
}
...
For multiple databases with NPX, pass `id=url` pairs:
  ...
  "mcpServers": {
    "postgres": {
      ...
      "args": [
        "-y",
        "multi-postgres-mcp-server",
        "db1=postgresql://localhost:5432/db_one",
        "db2=postgresql://localhost:5432/db_two"
      ]
    }
  }
}
